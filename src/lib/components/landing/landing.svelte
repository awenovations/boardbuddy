...
<style lang="scss">
	...
	.hero-bg-decoration {
		...
		height: 50rem;
		border-radius: 50%;
		background: radial-gradient(circle, var(--aura-secondary-40) 0%, transparent 70%);
		opacity: 0.15;
		filter: blur(80px);
		pointer-events: none;
	}
	...
	.cta-outlined {
		display: inline-flex;
		align-items: center;
		border: 2px solid var(--aura-tertiary-60);
		color: var(--aura-tertiary-60);
		padding: 1rem 2rem;
		border-radius: 0.5rem;
		font: var(--aura-default-semibold);
	...
	.feature-card {
		...
		&:hover {
			border-color: var(--aura-secondary-40);
	...
	.column-count {
		...
		height: 1.25rem;
		border-radius: 50%;
		background: var(--aura-container-border-color);
		color: white;
		font-size: 0.75rem;
	}
	...
	.cta-banner {
		max-width: 56rem;
		margin: 0 auto;
		background: linear-gradient(135deg, var(--aura-secondary-40), var(--aura-tertiary-60));
		border-radius: 1rem;
		padding: 3rem;
		overflow: hidden;
		position: relative;
